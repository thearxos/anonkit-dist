...
# Line-buffer output so progress shows immediately when launched from a GUI
# (a pipe would otherwise block-buffer and the window would look like it hung).
try:
...
DATA_DIR      = Path("/usr/share/anonkit")
BACKUP_DIR    = Path("/var/lib/anonkit/backups")
RULES_FILE    = DATA_DIR / "iptables.rules"
TORRC         = Path("/etc/tor/torrc")
SERVICE       = Path("/etc/systemd/system/tor-anonkit.service")
HARDEN_STATE  = BACKUP_DIR / "harden_state.json"
ORIG_HOSTNAME = BACKUP_DIR / "original_hostname"   # write-once real hostname (never a spoofed host-*)
...
def detect_backend() -> bool:
    ...
    # netfilter is usually a module that just isn't loaded yet - try to bring it up first
    for mod in ("nf_tables", "ip_tables", "iptable_nat", "iptable_filter"):
    ...
    for cmd, restore, is_nft in [
        ...
        # a real check: -L actually talks to the kernel netfilter subsystem
        if run(f"{cmd} -L -n 2>/dev/null", check=False).returncode == 0:
            ...
            return True
    err("No usable iptables backend: this kernel has no netfilter support")
    err("(nf_tables / ip_tables modules are absent). anonkit's transparent proxy needs it -")
    err("boot a kernel built with netfilter, or install its modules, then retry.")
    Backend.cmd, Backend.restore, Backend.is_nft = "iptables", "iptables-restore", False
    return False
...
def harden():
    ...
    #    TRANSIENT only: the real static /etc/hostname is never written, so a reboot (or a stop
    #    that never runs) auto-reverts and the real name can never be clobbered. The real name is
    #    saved WRITE-ONCE and we never capture an already-spoofed host-* as if it were real.
    real = run("hostnamectl --static hostname 2>/dev/null", check=False).stdout.strip()
    if real and not real.startswith("host-") and not ORIG_HOSTNAME.exists():
        try: ORIG_HOSTNAME.write_text(real)
        except Exception: pass
    state['hostname_spoofed'] = True
    rnd = ''.join(random.choices(string.ascii_lowercase + string.digits, k=8))
    new_hostname = f"host-{rnd}"
    run(f"hostnamectl --transient set-hostname {new_hostname} 2>/dev/null", check=False)
    ok(f"Hostname → {new_hostname}  (transient; real name preserved)")
...
def unharden():
    ...
    # Restore hostname: drop the transient (reverts to the real static /etc/hostname), and
    # re-assert the static name from the write-once backup in case an older build clobbered it.
    if state.get('hostname_spoofed') or state.get('hostname'):
        run("hostnamectl --transient set-hostname '' 2>/dev/null", check=False)
        real = ORIG_HOSTNAME.read_text().strip() if ORIG_HOSTNAME.exists() else state.get('hostname', '')
        if real and not real.startswith("host-"):
            run(f"hostnamectl --static set-hostname {real} 2>/dev/null", check=False)
        ok(f"Hostname restored → {real or '(static)'}")
...
def setup(bridge: Optional[str] = None, pin_guards: bool = False) -> bool:
    ...
    for d in (DATA_DIR, BACKUP_DIR, Path("/etc/anonkit")):
...
def start(bridge: Optional[str] = None, pin_guards: bool = False) -> bool:
    ...
    if not detect_backend():
        err("Aborting: the transparent proxy needs a working netfilter backend.")
        return False
    run("systemctl stop tor-anonkit.service 2>/dev/null", check=False)
    ...
    r = run("systemctl start tor-anonkit.service", check=False)
    if r.returncode != 0:
        err("Tor failed to start")
        run("journalctl -u tor-anonkit.service -n 20 --no-pager", check=False)
        remove_prelockdown()
        return False
    time.sleep(3)
...
def stop():
    ...
    detect_backend()
    run("systemctl stop tor-anonkit.service", check=False)
    ok("Tor stopped")
...
def new_circuit() -> bool:
    ...
    r = run("systemctl restart tor-anonkit.service", check=False)
    if r.returncode != 0:
        err("Restart failed")
        run("journalctl -u tor-anonkit.service -n 10 --no-pager", check=False)
        return False
    time.sleep(5)
    if run("systemctl is-active tor-anonkit.service", check=False).stdout.strip() != "active":
...
def status() -> bool:
    ...
    bs = run("journalctl -u tor-anonkit.service -n 3 --no-pager | grep -i bootstrap",
             check=False)
    if bs.stdout.strip():
...
def _snowflake_path() -> str:
    import shutil
    for p in ("/usr/bin/snowflake-client", "/usr/local/bin/snowflake-client",
              "/usr/bin/snowflake", os.path.expanduser("~/go/bin/snowflake-client")):
        if os.path.exists(p):
            return p
    return shutil.which("snowflake-client") or ""

def snowflake_bin() -> str:
    """Absolute path used in the torrc ClientTransportPlugin line."""
    return _snowflake_path() or "/usr/local/bin/snowflake-client"

def _invoking_user() -> str:
    """The real user behind sudo/pkexec (yay/makepkg refuse to run as root)."""
    import pwd
...
def ensure_snowflake() -> bool:
    if _snowflake_path():
        return True
    info("Installing the Snowflake client (one time, this can take a minute)...")
    user = _invoking_user()
    tries = []
    if user and user != "root":
        # -bin is precompiled (fast); yay/makepkg must run as a normal user
        tries.append(("snowflake-pt-client-bin (AUR)",
                      "sudo -u %s yay -S --noconfirm --needed snowflake-pt-client-bin" % user))
        tries.append(("snowflake-pt-client (AUR)",
                      "sudo -u %s yay -S --noconfirm --needed snowflake-pt-client" % user))
    # go build works even as root, as a last resort
    tries.append(("go build",
                  "command -v go >/dev/null 2>&1 && GOBIN=/usr/local/bin timeout 300 go install "
                  "gitlab.torproject.org/tpo/anti-censorship/pluggable-transports/snowflake/v2/"
                  "client@latest && ln -sf /usr/local/bin/client /usr/local/bin/snowflake-client"))
    for label, cmd in tries:
        info("trying %s ..." % label)
        run(cmd, check=False)
        if _snowflake_path():
            ok("snowflake-client ready (%s)" % label)
            return True
    err("could not install snowflake-client automatically.")
    err("install it once:  yay -S snowflake-pt-client-bin   then rerun  sudo anonkit --snowflake")
    return False
